...
authParentRouter.post("/login", (req, res) => {
  ...
  if (!phoneNumber && !password) {
    ...
    ParentModel.findOne({ phoneNumber })
      .then((parentFound) => {
        if (!parentFound)
          res.status(404).send({ success: 0, message: "No parent" });
        else {
          ...
          if (compare) {
            req.session.parent = {
              phoneNumber: parentFound.phoneNumber,
              name: parentFound.fullname,
              parentId: parentFound._id
            };
            res.send({ success: 0, message: "Login success", parentId: parentFound._id });
          } else
